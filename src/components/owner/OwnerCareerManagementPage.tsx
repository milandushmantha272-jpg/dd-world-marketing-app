...
import { Award, CheckCircle2, ClipboardCheck, Crown, DollarSign, Link2, RefreshCw, ShieldCheck, Users, XCircle } from 'lucide-react';
...
export const OwnerCareerManagementPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      for (const result of [u, t, r, a]) if (result.error) throw result.error;
      setEmployees(u.data || []); setTeams(t.data || []); setReviews(r.data || []); setAgreements(a.data || []);
  ...
  const saveReview = async () => {
    ...
    try {
      ...
      if (salaryAction === 'INCREASE') userPatch.salary_effective_from = new Date().toISOString().slice(0, 10);
  ...
  const saveCommissionAgreement = async () => {
    ...
    try {
      const { error: e } = await supabase.from('team_leader_commission_agreements').insert({
        ...
        owner_decided_at: commissionStatus === 'APPROVED' || commissionStatus === 'ACTIVE' ? new Date().toISOString() : null,
        decided_with_manager: true,
        permanent: true,
      });
      if (e) throw e;
      await supabase.from('users').update({ permanent_commission_eligible: true, permanent_commission_effective_from: new Date().toISOString().slice(0, 10) }).eq('id', commissionManagerId);
      setSuccess('Permanent Manager commission agreement recorded. Owner + Manager discussion is marked completed.');
  ...
  return <section className="dd-page-shell min-h-screen px-4 py-5 md:px-6 md:py-8">
    <div className="mx-auto w-full max-w-7xl space-y-5">
      ...
      <section className="rounded-3xl border border-amber-500/20 bg-amber-500/5 p-5">
        ...
        <p className="mt-2 text-xs leading-5 text-slate-400">Owner and the existing Team Manager can discuss the commission. The amount/percentage is stored as a permanent agreement and can only be made active through Owner decision.</p>
