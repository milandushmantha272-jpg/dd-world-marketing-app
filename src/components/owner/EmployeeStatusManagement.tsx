...
export const EmployeeStatusManagement: React.FC = () => {
  ...
  const filteredUsers = useMemo(() => users.filter((u) => {
    ...
    const searchOk = !query || u.name.toLowerCase().includes(query) || u.agentCode?.toLowerCase().includes(query) || u.employeeId?.toLowerCase().includes(query) || u.phone?.toLowerCase().includes(query);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-slate-900 via-rose-950/40 to-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
        ...
        <p className="text-xs text-slate-400 mt-2">Authentication credentials are managed only by Supabase Authentication. This screen controls employee status, approval and account lifecycle only.</p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-xl flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[240px]"><Search className="w-4 h-4 text-slate-500 absolute left-3.5 top-1/2 -translate-y-1/2" /><input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="නම / Agent Code / Employee ID / Phone" className="w-full bg-slate-950 border border-slate-800 rounded-2xl pl-10 pr-4 py-2.5 text-xs text-white outline-none" /></div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl overflow-x-auto">
        ...
        <table className="w-full text-left text-xs min-w-[1050px]">
          <thead><tr className="border-b border-slate-800 text-slate-400"><th className="p-3">Employee</th><th className="p-3">Phone</th><th className="p-3">ID</th><th className="p-3">Role</th><th className="p-3">Team</th><th className="p-3">Status</th><th className="p-3 text-right">Owner Action</th></tr></thead>
          <tbody className="divide-y divide-slate-800/60">{filteredUsers.map((u) => { const status: EmploymentStatus = u.employmentStatus || (u.status === 'blocked' ? 'BLOCKED' : 'ACTIVE'); return <tr key={u.id} className="hover:bg-slate-950/50"><td className="p-3"><div className="font-bold text-white">{u.name}</div><div className="text-[10px] text-slate-400">{u.email || 'Email not set'}</div></td><td className="p-3 font-mono text-emerald-300">{u.phone || u.mobile || 'Not added'}</td><td className="p-3 font-mono text-amber-300">{u.employeeId || u.agentCode || 'N/A'}</td><td className="p-3 text-slate-200">{u.role}</td><td className="p-3 text-slate-200">{u.teamName || 'Unassigned'}</td><td className="p-3 font-bold text-cyan-300">{status}</td><td className="p-3 text-right space-x-2">{status === 'ACTIVE' ? <button onClick={() => setStatus(u.id, 'SUSPENDED')} className="px-3 py-1.5 rounded-lg bg-amber-500/10 text-amber-300 border border-amber-500/20">Suspend</button> : <button onClick={() => setStatus(u.id, 'ACTIVE')} className="px-3 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-300 border border-emerald-500/20">Activate</button>}{status !== 'EXITED' && <button onClick={() => setStatus(u.id, 'EXITED')} className="px-3 py-1.5 rounded-lg bg-rose-500/10 text-rose-300 border border-rose-500/20">Exit</button>}<button onClick={() => removeEmployee(u.id)} className="px-3 py-1.5 rounded-lg bg-slate-800 text-slate-300"><Trash2 className="inline w-3.5 h-3.5" /></button></td></tr>; })}</tbody>
        </table>
      </div>
    </div>
  );
