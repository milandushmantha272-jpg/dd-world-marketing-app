...
type Props = { monthlyPresentation?: boolean; presentationSlides?: PresentationSlideData[] };
...
export const LiveMeetingRoom: React.FC<Props> = ({ monthlyPresentation = false, presentationSlides = [] }) => {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      if (data.team_id) { const { data: team } = await supabase.from('teams').select('name').eq('id', data.team_id).maybeSingle(); teamName = team?.name || null; }
  ...
  const attachRemote = (userId: string, stream: MediaStream) => { const el = remoteRefs.current[userId]; if (el && el.srcObject !== stream) { el.srcObject = stream; void el.play().catch(() => undefined); } };
  const sendSignal = async (event: string, to: string, payload: any) => { const channel = channelRef.current; if (!channel || !me) return; await channel.send({ type: 'broadcast', event: 'webrtc', payload: { event, from: me.userId, to, payload } }); };
  const broadcastPresentation = async (index: number, slides: PresentationSlideData[] = liveSlides) => { if (!channelRef.current || !isOwner || !monthlyPresentation) return; await channelRef.current.send({ type: 'broadcast', event: 'presentation', payload: { type: 'state', index, slides } }); };
  ...
  const createPeer = (remote: Participant, initiator: boolean) => {
    ...
    if (initiator) void pc.createOffer().then(offer => pc.setLocalDescription(offer)).then(() => { const desc = pc.localDescription; if (desc) return sendSignal('offer', remote.userId, desc); return undefined; }).catch(() => undefined);
    return pc;
  };

  const handleSignal = async ({ payload }: any) => {
    if (!me || !payload || (payload.to && payload.to !== me.userId)) return;
    const remoteId = payload.from as string; if (remoteId === me.userId) return;
    const remote = participantsRef.current[remoteId]; if (!remote) return;
    const pc = createPeer(remote, false); if (!pc) return;
    try {
      if (payload.event === 'offer') { await pc.setRemoteDescription(payload.payload); const answer = await pc.createAnswer(); await pc.setLocalDescription(answer); const desc = pc.localDescription; if (desc) await sendSignal('answer', remoteId, desc); }
  ...
  const syncParticipants = (state: Record<string, any[]>) => {
    ...
    participantsRef.current = Object.fromEntries(list.map(p => [p.userId, p])); setParticipants(list);
  ...
  const handlePresentation = async ({ payload }: any) => {
    ...
    if (payload.type === 'state' && !isOwner) { if (Array.isArray(payload.slides)) setLiveSlides(payload.slides); if (Number.isFinite(payload.index)) setSlideIndex(Math.max(0, Number(payload.index))); }
  };
  ...
  const startMeeting = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('live_meeting_rooms').insert({ room_code: code, title: monthlyPresentation ? 'DD WORLD Monthly Presentation & Meeting' : 'DD WORLD Live Meeting', meeting_type: monthlyPresentation ? 'MONTHLY_PRESENTATION' : 'GENERAL', created_by: me.userId, status: 'live', starts_at: new Date().toISOString() }).select('id,room_code,title,status').single();
      if (error) throw error;
      setActiveRoom(data); setRoomCode(data.room_code); await joinRoom(data.room_code, data.title);
    } catch (e: any) { setNotice(e?.message || 'Meeting could not be started.'); } finally { setBusy(false); }
  };

  const joinRoom = async (codeInput?: string, title = 'DD WORLD Live Meeting') => {
    if (!me) return;
    const code = (codeInput || roomCode).trim().toUpperCase(); if (!code) { setNotice('Enter the meeting code.'); return; }
    setBusy(true); setNotice(null);
    try {
      ...
      if (error) throw error; if (!room || room.status !== 'live') throw new Error('This meeting is not live.');
      ...
      const channel: any = supabase.channel(`dd-world-live-meeting:${code}`, { config: { presence: { key: me.userId } } });
      channel.on('presence', { event: 'sync' }, () => syncParticipants(channel.presenceState())).on('presence', { event: 'join' }, () => syncParticipants(channel.presenceState())).on('presence', { event: 'leave' }, () => syncParticipants(channel.presenceState())).on('broadcast', { event: 'webrtc' }, handleSignal).on('broadcast', { event: 'chat' }, ({ payload }: any) => { if (payload?.userId !== me.userId) setChat(prev => [...prev, payload]); }).on('broadcast', { event: 'presentation' }, handlePresentation);
      const status = await channel.subscribe();
      if (status !== 'SUBSCRIBED') throw new Error('Live meeting connection failed.');
      channelRef.current = channel; await channel.track({ userId: me.userId, name: me.name, role: me.role, team: me.team, joinedAt: new Date().toISOString() });
    ...
    } catch (e: any) { streamRef.current?.getTracks().forEach(t => t.stop()); streamRef.current = null; setNotice(e?.message || 'Camera/microphone permission is required.'); }
    finally { setBusy(false); }
  };

  const cleanupMeeting = () => {
    (Object.values(peers.current) as RTCPeerConnection[]).forEach(pc => pc.close()); peers.current = {};
    if (channelRef.current) { void supabase.removeChannel(channelRef.current); channelRef.current = null; }
    streamRef.current?.getTracks().forEach(t => t.stop()); streamRef.current = null; if (localVideoRef.current) localVideoRef.current.srcObject = null;
  };
  const leaveMeeting = async () => { cleanupMeeting(); setParticipants([]); setChat([]); setSharing(false); setActiveRoom(null); };
  const endMeeting = async () => { if (!activeRoom || !isOwner) return; await supabase.from('live_meeting_rooms').update({ status: 'ended', ended_at: new Date().toISOString() }).eq('id', activeRoom.id); await leaveMeeting(); };
  const setPresentationSlide = async (nextIndex: number) => { if (!isOwner || !liveSlides.length) return; const safe = Math.min(Math.max(nextIndex, 0), liveSlides.length - 1); setSlideIndex(safe); await broadcastPresentation(safe, liveSlides); };
  ...
  const shareScreen = async () => {
    ...
    const peersList = Object.values(peers.current) as RTCPeerConnection[];
    if (sharing) { const camera = await navigator.mediaDevices.getUserMedia({ video: true, audio: false }); const next = camera.getVideoTracks()[0]; const old = streamRef.current.getVideoTracks()[0]; peersList.forEach(pc => { const sender = pc.getSenders().find(s => s.track?.kind === 'video'); if (sender) void sender.replaceTrack(next); }); old?.stop(); if (old) streamRef.current.removeTrack(old); streamRef.current.addTrack(next); if (localVideoRef.current) localVideoRef.current.srcObject = streamRef.current; setSharing(false); return; }
    const screen = await navigator.mediaDevices.getDisplayMedia({ video: true }); const next = screen.getVideoTracks()[0]; const old = streamRef.current.getVideoTracks()[0]; peersList.forEach(pc => { const sender = pc.getSenders().find(s => s.track?.kind === 'video'); if (sender) void sender.replaceTrack(next); }); next.onended = () => setSharing(false); old?.stop(); if (old) streamRef.current.removeTrack(old); streamRef.current.addTrack(next); if (localVideoRef.current) localVideoRef.current.srcObject = streamRef.current; setSharing(true);
  };
  const sendChat = async () => { if (!chatText.trim() || !channelRef.current || !me) return; const item = { id: crypto.randomUUID(), userId: me.userId, name: me.name, text: chatText.trim(), at: new Date().toISOString() }; await channelRef.current.send({ type: 'broadcast', event: 'chat', payload: item }); setChat(prev => [...prev, item]); setChatText(''); };
  ...
  return <div className="space-y-5 rounded-3xl border border-indigo-500/20 bg-slate-950 p-4 md:p-6">
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><div className="flex items-center gap-2"><Video className="h-6 w-6 text-indigo-400" /><h2 className="text-lg font-black text-white">{monthlyPresentation ? 'Monthly Presentation + Live Meeting' : 'Live Meeting Room'}</h2></div><p className="mt-1 text-xs text-slate-400">Real camera + microphone WebRTC session • Owner / Team Leaders / Agents</p></div><div className="flex flex-wrap gap-2">{isOwner && !activeRoom && <button onClick={() => void startMeeting()} disabled={busy} className="rounded-xl bg-indigo-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50"><Play className="mr-1 inline h-4 w-4" />Start Live Meeting</button>}{!activeRoom && !isOwner && <><input value={roomCode} onChange={e => setRoomCode(e.target.value)} placeholder="Meeting code" className="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2.5 text-xs text-white" /><button onClick={() => void joinRoom()} disabled={busy} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">Join Live</button></>}{activeRoom && <button onClick={() => void navigator.clipboard?.writeText(roomCode)} className="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2.5 text-xs font-bold text-slate-200"><Copy className="mr-1 inline h-4 w-4" />Copy Code</button>}</div></div>
    ...
    {!activeRoom ? <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 text-center text-xs text-slate-400">{isOwner ? 'Owner can start a protected live room. Share the meeting code through Message Room.' : 'Enter the meeting code provided by Owner and join the live room.'}</div> : <><div className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-3"><div className="flex items-center gap-2 text-xs font-black text-white"><span className="h-2.5 w-2.5 animate-pulse rounded-full bg-emerald-400" />LIVE • {activeRoom.title} • {roomCode}</div><span className="text-xs text-slate-400"><Users className="mr-1 inline h-4 w-4" />{participants.length} online</span></div><div className="grid grid-cols-1 gap-4 lg:grid-cols-4"><div className="lg:col-span-3 grid grid-cols-2 gap-3 md:grid-cols-3"><div className="relative aspect-video overflow-hidden rounded-2xl border border-indigo-500/30 bg-slate-900"><video ref={localVideoRef} muted playsInline className="h-full w-full object-cover" /><span className="absolute bottom-2 left-2 rounded-lg bg-black/60 px-2 py-1 text-[10px] font-bold text-white">{me.name} • You</span></div>{participants.filter(p => p.userId !== me.userId).map(p => <div key={p.userId} className="relative aspect-video overflow-hidden rounded-2xl border border-slate-800 bg-slate-900"><video playsInline autoPlay ref={el => { remoteRefs.current[p.userId] = el; }} className="h-full w-full object-cover" /><div className="absolute inset-0 -z-0 flex items-center justify-center text-xs text-slate-500">Waiting for video…</div><span className="absolute bottom-2 left-2 rounded-lg bg-black/60 px-2 py-1 text-[10px] font-bold text-white">{p.name} • {p.role.replace('_',' ')}</span></div>)}</div><div className="flex min-h-[300px] flex-col rounded-2xl border border-slate-800 bg-slate-900"><div className="border-b border-slate-800 p-3 text-xs font-black text-white"><MessageSquare className="mr-1 inline h-4 w-4 text-indigo-400" />Live Chat</div><div className="flex-1 space-y-2 overflow-y-auto p-3">{chat.map(c => <div key={c.id} className="rounded-xl bg-slate-950 p-2.5"><div className="text-[10px] font-bold text-indigo-300">{c.name}</div><div className="text-xs text-slate-300">{c.text}</div></div>)}</div><div className="flex gap-2 border-t border-slate-800 p-2"><input value={chatText} onChange={e => setChatText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') void sendChat(); }} placeholder="Message…" className="min-w-0 flex-1 rounded-xl bg-slate-950 px-3 py-2 text-xs text-white" /><button onClick={() => void sendChat()} className="rounded-xl bg-indigo-600 px-3 text-xs font-black text-white">Send</button></div></div></div><div className="flex flex-wrap items-center justify-center gap-2 rounded-2xl border border-slate-800 bg-slate-900 p-3"><button onClick={toggleMic} className={`rounded-xl p-3 ${micOff ? 'bg-red-600 text-white' : 'bg-slate-800 text-slate-200'}`}>{micOff ? <MicOff className="h-5 w-5" /> : <Mic className="h-5 w-5" />}</button><button onClick={toggleVideo} className={`rounded-xl p-3 ${videoOff ? 'bg-red-600 text-white' : 'bg-slate-800 text-slate-200'}`}>{videoOff ? <VideoOff className="h-5 w-5" /> : <Video className="h-5 w-5" />}</button><button onClick={() => void shareScreen()} className={`rounded-xl p-3 ${sharing ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-200'}`}><MonitorUp className="h-5 w-5" /></button><button onClick={() => void leaveMeeting()} className="rounded-xl bg-red-600 p-3 text-white"><PhoneOff className="h-5 w-5" /></button>{isOwner && <button onClick={() => void endMeeting()} className="rounded-xl bg-rose-800 px-4 py-3 text-xs font-black text-white"><Square className="mr-1 inline h-4 w-4" />End Meeting</button>}</div></>}
  </div>;
};
