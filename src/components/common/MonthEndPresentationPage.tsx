...
export const MonthEndPresentationPage: React.FC<{ embedded?: boolean }> = ({ embedded = false }) => {
  ...
  return <div className="space-y-5 p-3 pb-12 md:p-5">
    ...
    {isOwner ? <>
      ...
      <section className="rounded-3xl border border-slate-800 bg-slate-900 p-5"><div className="flex items-center gap-2 text-sm font-black text-white"><Users className="h-5 w-5 text-cyan-400" />TEAM SUMMARY</div><div className="mt-4 space-y-2">{teamStats.map((t: any, i: number) => <div key={t.team.id} className="grid grid-cols-2 gap-2.5 sm:gap-3 gap-2 rounded-xl border border-slate-800 bg-slate-950 p-3 text-xs md:grid-cols-5"><div className="font-bold text-white">#{i + 1} {t.team.name}</div><div>Sales <b className="text-cyan-300">{t.total}</b></div><div>Sayuru <b>{t.sayuru}</b></div><div>Govi <b>{t.govi}</b></div><div>Best <b className="text-emerald-300">{t.best?.agent.name || '—'}</b></div></div>)}</div></section>
