...
export const MessageRoomPage: React.FC = () => {
  ...
  const [tab, setTab] = useState<'messages' | 'meetings'>('messages');
  ...
  useEffect(() => {
    void load();
    const channel = supabase.channel('dd-world-message-room')
  ...
  const send = async (category: string = 'message') => {
    ...
    const { error } = await supabase.from('messages').insert({
      id: crypto.randomUUID(), sender_id: me.id, receiver_id: receiver,
      message: text.trim(), title: title.trim() || null, category,
      timestamp: new Date().toISOString(), read: false
    });
    if (error) setStatus(`Send failed: ${error.message}`);
    else { setStatus('Message sent ✓'); setText(''); setTitle(''); await load(); }
  };
  ...
  return <div className="space-y-5 p-3 md:p-5">
    ...
    {tab === 'meetings' ? <MonthEndPresentationPage embedded /> : <>
      <div className="rounded-3xl border border-slate-800 bg-slate-900 p-5 space-y-4">
        ...
        <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Title (optional)" className="w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm text-white" />
        <textarea value={text} onChange={e => setText(e.target.value)} placeholder="Write message..." rows={3} className="w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm text-white" />
        <div className="flex gap-2"><button disabled={!receiver || !text.trim()} onClick={() => void send()} className="rounded-xl bg-emerald-600 px-5 py-3 text-xs font-black text-white disabled:opacity-40"><Send className="mr-2 inline h-4 w-4" />Send</button><button disabled={!receiver || !text.trim()} onClick={() => void send('meeting')} className="rounded-xl bg-indigo-600 px-5 py-3 text-xs font-black text-white disabled:opacity-40"><Video className="mr-2 inline h-4 w-4" />Send Meeting Notice</button></div>
        {status && <p className="text-xs font-bold text-emerald-300">{status}</p>}
      </div>
