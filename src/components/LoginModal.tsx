...
export const LoginModal: React.FC = () => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const target = users.find((user) =>
      user.id.toLowerCase() === value || user.agentCode?.trim().toLowerCase() === value || user.employeeId?.trim().toLowerCase() === value || user.email?.trim().toLowerCase() === value,
    );
    if (!target && !value.includes('@')) { setError('Employee account එක හමු නොවීය. Email එකෙන් login කරන්න.'); return; }
    if (target && target.role !== selectedRole) { setError(`මෙම account එක ${roleMeta[target.role]?.label || target.role} role එකට අයත්ය.`); return; }
    if (target && (blockedStatuses.has(String(target.employmentStatus || '')) || target.status === 'blocked')) { setError('මෙම account එක Owner විසින් BLOCK / SUSPEND / EXIT කර ඇත. Login denied.'); return; }
    if (target && !target.email?.trim()) { setError('මෙම employee account එකට email එකක් සකසා නැත.'); return; }
    ...
    try {
      ...
      setError(err?.message || 'Supabase authentication අසාර්ථකයි.');
    } finally { setBusy(false); }
  };

  const resetOwnerPassword = async () => {
    setError(''); setResetSent(false); setResetBusy(true);
    try { await sendOwnerPasswordReset(); setResetSent(true); }
    catch (err: any) { setError(err?.message || 'Owner password reset email එක යැවීමට නොහැකි විය.'); }
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/95 p-4 backdrop-blur-md overflow-y-auto">
    <div className="w-full max-w-md rounded-3xl border border-red-500/30 bg-slate-900 p-6 shadow-2xl shadow-red-950/30 sm:p-8">
      ...
      <div className="mb-6 text-center"><h1 className="text-2xl font-black text-white">DD WORLD MARKETING</h1><p className="mt-1 text-xs text-slate-400">Secure Supabase Authentication</p></div>
      <div className="mb-5 flex items-center justify-center gap-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs font-bold text-emerald-300"><ShieldCheck className="h-4 w-4" /> Owner-controlled Authentication</div>
      ...
      {selectedRole === 'owner' && <div className="mb-4 rounded-xl border border-blue-500/30 bg-blue-500/10 p-3 text-xs text-blue-100"><b>Owner:</b> use the registered Owner email and Supabase password. Forgot password සඳහා secure email reset භාවිතා කරන්න.</div>}
      <form onSubmit={submit} className="space-y-4">
        <label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-300">Employee ID / Agent Code / Email</span><input value={identifier} onChange={(e) => setIdentifier(e.target.value)} autoComplete="username" className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white outline-none focus:border-red-500" placeholder="Enter employee identifier or email" /></label>
      ...
      <p className="mt-5 text-center text-[10px] leading-relaxed text-slate-500">Supabase Auth + database RLS are authoritative. Blocked, suspended, rejected or inactive employees are denied before app access.</p>
    </div>
  </div>;
};
