export type UserRole = 'owner' | 'team_leader' | 'junior_team_leader' | 'agent' | 'dialog_officer';
...
export interface User {
  id: string; name: string; email?: string; role: UserRole; authUserId?: string; firebaseUid?: string; agentCode?: string; nic?: string; phone?: string; mobile?: string; teamId?: string; teamName?: string; teamLeaderId?: string; teamLeaderName?: string; avatar?: string; employmentStatus?: EmploymentStatus; joinedDate?: string; createdAt?: string; designation?: string; workLocation?: string; district?: string; address?: string; emergencyContact?: string; emergencyContactName?: string; drivingLicenceNumber?: string; drivingLicenceExpiry?: string; annualVerificationDate?: string; annualVerificationExpiry?: string; isAppDownloaded?: boolean; isLoggedIn?: boolean; lastLoginAt?: string; appVersion?: string; jobPosition?: string; idApprovalStatus?: EmployeeIdApprovalStatus; idApprovedAt?: string; idApprovedBy?: string; ownerSignatureUrl?: string; idRejectedReason?: string; approvedByOwner?: boolean; status?: string; employeeId?: string;
  baseSalary?: number; salaryEffectiveFrom?: string; nextSalaryReviewAt?: string; careerStage?: string; reportsToUserId?: string; managementRole?: string; permanentCommissionEligible?: boolean; permanentCommissionEffectiveFrom?: string;
  kycDocuments?: { gnCertificate?: string; policeReport?: string; nicFront?: string; nicBack?: string; drivingLicenceFront?: string; drivingLicenceBack?: string; employeePhoto?: string; }; gramaNiladhariReportUrl?: string; policeReportUrl?: string; gramaReportName?: string; policeReportName?: string; gramaReportUploadedAt?: string; policeReportUploadedAt?: string;
}
...
export type AppActivationStatus = 'PENDING' | 'CUSTOMER_CONFIRMED' | 'COMPLETED' | 'REJECTED' | 'DUPLICATE';
export interface ProductSale { id:string; agentId:string; agentCode:string; agentName:string; teamId?:string; productType:'govimithuru'|'sayuru'|string; productName?:string; channel?:string; quantity?:number; msisdn?:string; customerName?:string; customerMobile?:string; location?:string; latitude?:number; longitude?:number; gpsAccuracy?:number; district?:string; saleDate?:string; date?:string; time?:string; status?:string; verificationStatus?:string; verifiedAt?:string; verifiedBy?:string; verificationNote?:string; amount?:number; notes?:string; activationMethod?:'KEYPAD_DIAL'|'APP_LINK_SHARE'|'MANUAL'; dialCode?:string; appShareChannel?:'WHATSAPP'|'SMS'|'QR'|'DIRECT'; trackingToken?:string; trackingProductKey?:'sayuru'|'govimithuru'; activationStatus?:AppActivationStatus; customerConfirmedAt?:string; customerConfirmedBy?:string; duplicateDetectedAt?:string; }
...
export interface DialogReportSummaryEntry { agentCode?: string; agentName: string; productType: 'ගොවිමිතුරු' | 'සයුරු'; channel: 'IVR' | 'APP' | 'TOTAL'; reportCount: number; }
export interface DialogReportSummary { id: string; reportType: 'govimithuru' | 'sayuru'; periodStart: string; periodEnd: string; attachedAt: string; entries: DialogReportSummaryEntry[]; }
export interface ReportReconciliationResult { agentKey: string; agentName: string; productType: 'ගොවිමිතුරු' | 'සයුරු'; channel: 'IVR' | 'APP'; appCount: number; dialogCount: number; matchedCount: number; difference: number; status: 'MATCHED' | 'DIALOG_MORE' | 'APP_MORE' | 'NO_DIALOG_DATA'; }
